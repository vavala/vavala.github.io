...
function BibtexDisplay() {
  ...
  this.displayAuthor = function(string){
	...
	var authorname = "";
	authorname += arrayString[0].substr(arrayString[0].indexOf(',')+1);
	authorname += " ";
	authorname += arrayString[0].substr(0, arrayString[0].indexOf(','));  
	var newString = authorname;
  	for (i = 1; i < arrayString.length; i++) {
	  //arraystring is "lastname, firstname"
	  //so remove any space and make "firstname lastname"
	  //var authorname = arrayString[i];
	  authorname = "";
	  authorname += arrayString[i].substr(arrayString[i].indexOf(',')+1);
	  authorname += " ";
	  authorname += arrayString[i].substr(0, arrayString[i].indexOf(','));  	  
	  if(i+1>=arrayString.length) {
  	    newString += ", and " + authorname;
  	  } else {
	    newString += ", " + authorname;
	  }
	}
	return newString;
  }
